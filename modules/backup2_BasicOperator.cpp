...
void BasicOperator::init_operator (const int t_source, const int t_sink){
  ...
  for(int rnd_i = 0; rnd_i < number_of_rnd_vec; ++rnd_i)
    for(int dirac = 0; dirac < 16; dirac = dirac + 4)
 ...
 //   4 * number_of_eigen_vec, 4 * number_of_eigen_vec);
  ...
  switch(dirac) {
    ...
    case 5:
      ...
      (op_tsource_tsink[rnd_i]).block(0, 1 * number_of_eigen_vec, 4 * number_of_eigen_vec, number_of_eigen_vec) = contraction[rnd_i][4];
      (op_tsource_tsink[rnd_i]).block(0, 2 * number_of_eigen_vec, 4 * number_of_eigen_vec, number_of_eigen_vec) = -1 * contraction[rnd_i][8];
      (op_tsource_tsink[rnd_i]).block(0, 3 * number_of_eigen_vec, 4 * number_of_eigen_vec, number_of_eigen_vec) = -1 * contraction[rnd_i][12];

      (op_tsink_tsource[rnd_i]).block(0, 0                      , 4 * number_of_eigen_vec, number_of_eigen_vec) = contraction2[rnd_i][0];
      (op_tsink_tsource[rnd_i]).block(0, 1 * number_of_eigen_vec, 4 * number_of_eigen_vec, number_of_eigen_vec) = contraction2[rnd_i][4];
      (op_tsink_tsource[rnd_i]).block(0, 2 * number_of_eigen_vec, 4 * number_of_eigen_vec, number_of_eigen_vec) = -1 * contraction2[rnd_i][8];
      (op_tsink_tsource[rnd_i]).block(0, 3 * number_of_eigen_vec, 4 * number_of_eigen_vec, number_of_eigen_vec) = -1 * contraction2[rnd_i][12];
