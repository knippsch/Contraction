...
LapH::Perambulator::Perambulator () : perambulator() {
  ...
  // TODO: The size should be changed dynamically and not fixed to be 10!! This
  //       could lead to a bug.
  ...
  for(const auto& q: quarks){
    ...
    for(int i = 0; i < number_of_rnd_vec; ++i){
      perambulator[q.id][i] = 
          Eigen::MatrixXcd::Zero(4 * nb_ev * Lt, nb_i);
    }
  }
...
//            number_of_inversions * Lt, 2 * Lt * number_of_eigen_vec * 4);
        ...
        int blabla = fread(&(perambulator_read[0]), 
        ...
        for(t1 = 0; t1 < Lt; ++t1)
          for(ev1 = 0; ev1 < number_of_eigen_vec; ++ev1)
            for(dirac1 = 0; dirac1 < 4; ++dirac1)
              for(t2 = 0; t2 < (Lt / q.number_of_dilution_T); ++t2)
                for(ev2 = 0; ev2 < q.number_of_dilution_E; ++ev2)
                  for(dirac2 = 0; dirac2 < q.number_of_dilution_D; ++dirac2){
                    ...
                    perambulator[q.id][rnd_vec_i](4 * number_of_eigen_vec * t1+
                        number_of_eigen_vec * dirac1 + ev1, 
                        q.number_of_dilution_E * 
                          q.number_of_dilution_D * t2 + 
